...
export default function App() {
  ...
  async function ask(value = question) {
    ...
    try {
      ...
      const result = await askLatest(trimmed, lang, release.activeVersion, matches);
      setMessages((items) => [...items, { answerId: crypto.randomUUID(), question: trimmed, answer: result.answer, source: matches.length ? matches.map((match) => match.heading).join(' · ') : null, sectionIds: matches.map((match) => match.id), language: lang, guideVersion: release.activeVersion }]); setQuestion('');
    } catch (error) { console.error(error); } finally { setAsking(false); }
  }
